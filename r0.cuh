...
__global__ void gemm_bf16_naive_kernel_b(const __nv_bfloat16 *__restrict__ A,
  ...
  float acc = 0.0f;
  for (int k = 0; k < K; ++k) {
   acc += static_cast<float>(A[row + k * M] * B[k + col * K]);
  }

  C[row + col * M] = acc;
}
// Simple naive kernel with 1d block, memory coalescing attempt, but results similar to previous kernels
...
template <int TILE>
__global__ void gemm_bf16_naive_kernel_d(
    ...
    const int M, const int N, const int K)
{
    ...
    float sum = 0.0f;
    // #pragma unroll
    // Pragma unroll is not effective for bf16 data types, infact perfomace is lost
    // ldg also ineffective, perfomance similar to above
    for (int k = 0; k < K; ++k) {
        sum += static_cast<float>(__ldg(&A[row + k * M]) * __ldg(&B[k + col * K]));
    }
    C[row * N + col] = sum;
    }
