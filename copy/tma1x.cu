// file: tma1.cu  (2D TMA load/store, fixed)
...
// Suppress benign warning about dynamic init of __shared__ barrier;
// we explicitly init() it before use.
    ...
    int M, int N)
{
    ...
    // ---- Init mbarrier: ONE expected arrival (only the issuer arrives) ----
    if (tid == 0) {
        init(&bar, 1);
        fence_proxy_async();
    }
    ...
    // ---- ISSUE: single thread launches the bulk copy + tx-count arrival ----
    if (tid == 0) {
    ...
    // ---- WAIT: every thread waits on phase 0 (no personal token needed) ----
    bar.wait_parity(0);

    // ---- Compute on the tile (multiply by 2.0f in-place) ----
    ...
    // ---- Store back via TMA ----
    // Fence required: we just wrote smem with the generic proxy, now
    // we want the async proxy (TMA) to read it.
    if (tid == 0) {
        fence_proxy_async();
...
// --------- Driver ---------
int main() {
    // Sanity: confirm we're on Hopper or newer.
    int dev = 0;
    cudaDeviceProp prop{};
